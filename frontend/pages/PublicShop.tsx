...
import { MessageCircle, ShoppingBag, Info, AlertTriangle } from 'lucide-react';
...
const PublicShop: React.FC = () => {
  ...
  const sendWhatsApp = (product: Product) => {
    ...
    const message = encodeURIComponent(`Bonjour ! Je suis intéressé par votre produit : ${product.name} (${product.price}€). Est-il disponible ?`);
    const cleanNumber = shop.whatsapp_number.replace(/\s+/g, '').replace('+', '');
  ...
  if (error || !shop) return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center">
      ...
      <p className="text-gray-500 mt-2">Le lien semble incorrect ou la boutique n'existe plus.</p>
      <a href="/" className="mt-8 text-indigo-600 font-bold underline">Retourner à l'accueil</a>
    </div>
  );
    ...
    <div className="min-h-screen bg-white">
      ...
      <header className="py-12 px-6 text-center bg-gray-50 border-b">
        <div className="max-w-3xl mx-auto">
          {shop.logo ? (
            <img src={shop.logo} alt={shop.name} className="w-24 h-24 rounded-full mx-auto mb-6 object-cover shadow-md" />
          ) : (
          ...
          <p className="text-gray-500 max-w-lg mx-auto">{shop.description}</p>
        </div>
      </header>
        ...
        {products.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((product) => (
              <div key={product.id} className="bg-white rounded-3xl border border-gray-100 overflow-hidden shadow-sm hover:shadow-xl transition-all flex flex-col h-full">
                ...
                <div className="p-6 flex flex-col flex-1">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span className="text-indigo-600 font-bold text-xl whitespace-nowrap ml-2">{product.price}€</span>
                  </div>
      ...
      <footer className="py-12 px-6 border-t bg-gray-50 text-center">
        <div className="flex flex-col items-center gap-4">
          <div className="flex items-center gap-2 text-gray-400 text-sm">
            ...
            <div className="w-6 h-6 bg-indigo-600 rounded flex items-center justify-center text-white font-bold text-[10px]">LC</div>
            <span className="font-bold text-gray-600">LinkContact</span>
          </div>
          <p className="text-xs text-gray-400">Créez votre propre boutique en 2 minutes.</p>
        </div>
      </footer>

      {/* Bottom Sticky Info for Mobile */}
